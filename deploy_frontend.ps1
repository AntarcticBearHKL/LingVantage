...
Set-Location -Path "front_end/next_app"
...
Remove-Item -Path "../../back_end/api/wwwroot/*" -Recurse -Force

# 5. Copy build results
Write-Host "Copying static files to wwwroot..."
Copy-Item -Path "out/*" -Destination "../../back_end/api/wwwroot/" -Recurse -Force
